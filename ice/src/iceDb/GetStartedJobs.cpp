...
namespace { // begin local namespace
    ...
    static int fetch_fields_callback(void *param, int argc, char **argv, char **azColName) {
      ...
      vector< boost::tuple< time_t, std::string, std::string, std::string > >* result = (vector< boost::tuple< time_t, std::string, std::string, std::string > >*) param;
...
void db::GetStartedJobs::execute( sqlite3* db ) throw ( DbOperationException& )
{
  string sqlcmd("SELECT timestamp,ceurl,gridjobid,creamjobid FROM started_jobs WHERE timestamp >= ");
